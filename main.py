...
# Motor / Gearing
AMK_Eff_Drive = 0.80    # Efficiency with inverter->motor->gearbox (%)
AMK_Eff_Regen = 0.65    # Efficiency with gearbox->motor->inverter (%)
Emrax_Eff_Drive = 0.85  # Efficiency with inverter->motor->chain (%)
Emrax_Eff_Regen = 0.70  # Efficiency with chain->motor->inverter (%)
Max_Power = 80          # (kWh)
...
# HELPER FUNCTIONS
def Cell_kWH_to_volts(kWh): #Cell voltage from energy (kWh) used
    mAH = kWh / Energy_battery_max * cell_charge_tbl[0]
    left = 0
    right = CELL_CHGR_ARR_SIZE - 1
    mid = (left + right) // 2
    while right - left > 1:
        if mAH > cell_charge_tbl[CELL_CHGR_ARR_SIZE - 1 - mid]:
            left = mid
        elif mAH < cell_charge_tbl[CELL_CHGR_ARR_SIZE - 1 - mid]:
            right = mid
        else:
            return cell_volts_tbl[CELL_CHGR_ARR_SIZE - 1 - mid]
        mid = (left + right) // 2
    return cell_volts_tbl[left] + (cell_volts_tbl[right] - cell_volts_tbl[left]) * (mAH - cell_charge_tbl[CELL_CHGR_ARR_SIZE - 1 - left]) / (cell_charge_tbl[CELL_CHGR_ARR_SIZE - 1 - right] - cell_charge_tbl[CELL_CHGR_ARR_SIZE - 1 - left])
...
def main():
    track = pd.read_csv(TRACK_CSV) # read track layout
    total_time = 0
    entry_speed = 0
    ### testing
    for i in range(0, 70):
        print(i, Cell_kWH_to_volts(i*0.1))
