...
def Drivetrain_Model(commandedPower, acc_x, acc_y, velocity):
    ...
    Fz_Downforce = -LIFT_COEFF*1/2*AIR_DENSITY*ACS_FRONT*velocity**2
...
def main():
    ...
    # Plot time vs power
    time = [state.time for state in car_properties]
    power = [state.tot_power for state in car_properties]
    fz = [(state.Fz_rl + state.Fz_rr + state.Fz_fl + state.Fz_fr) for state in car_properties]

    plt.figure(figsize=(10, 6))
    plt.plot(time, fz, label='Net Fz (4 wheels)')
    plt.xlabel('Time (s)')
    plt.ylabel('Fz (N)')
